...
async function getPrxList(prxBankUrl = PRX_BANK_URL, countryCode = null) {
  ...
  if (countryCode) {
    prxBankUrl = `https://raw.githubusercontent.com/maztampandata/cfproxies/main/proxies/${countryCode}.txt`;
  }
...
// Fungsi reverseWeb telah dihapus

// Fungsi getAllConfig telah dihapus
    ...
    try {
      ...
      if (upgradeHeader === "websocket") {
        ...
        if (url.pathname.length == 3 || url.pathname.match(",")) {
          ...
          const countryCode = prxKeys[Math.floor(Math.random() * prxKeys.length)];
          const prxList = await getPrxList(PRX_BANK_URL, countryCode);
          
          if (prxList.length > 0) {
            const randomProxy = prxList[Math.floor(Math.random() * prxList.length)];
      ...
      // Endpoint /sub telah dihapus karena menampilkan web
