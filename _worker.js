...
async function getProxyData(prxList, limit = null) {
  const result = [];
  ...
  for (let i = 0; i < Math.min(maxResults, prxList.length); i++) {
    ...
    let pingInfo = null;
    try {
      const pingResult = await checkPrxHealth(prxIP, prxPort);
      pingInfo = {
        status: pingResult.status || "UNKNOWN",
        delay: pingResult.delay || null,
        colo: pingResult.colo || null
      };
    } catch (error) {
      pingInfo = {
        status: "ERROR",
    ...
    // Generate TLS and non-TLS configurations
    ...
    for (const protocol of PROTOCOLS) {
      ...
      if (protocol === "ss") {
        ...
        uri.searchParams.set(
          "plugin",
          `${atob(v2)}-plugin;tls;mux=0;mode=websocket;path=/${prxIP}-${prxPort};host=${APP_DOMAIN}`
        );
      } else {
        uri.username = uuid;
    ...
    // Non-TLS configurations (port 80)
    for (const protocol of PROTOCOLS) {
      const uri = new URL(`${atob(horse)}://${APP_DOMAIN}`);
      uri.port = "80";
      uri.searchParams.set("encryption", "none");
      ...
      uri.searchParams.set("sni", protocol === atob(flash) ? "" : APP_DOMAIN);
      
      if (protocol === "ss") {
        uri.username = btoa(`none:${uuid}`);
        uri.searchParams.set(
          "plugin",
          `${atob(v2)}-plugin;mux=0;mode=websocket;path=/${prxIP}-${prxPort};host=${APP_DOMAIN}`
        );
      } else {
        uri.username = uuid;
        uri.searchParams.delete("plugin");
    ...
    result.push({
      ...
      port: prxPort,
      country: country,
      country_flag: getFlagEmoji(country),
      isp: org,
      ping: pingInfo,
      configs: configs
    });
...
export default {
  async fetch(request, env, ctx) {
    try {
      ...
      if (url.pathname.startsWith("/api/proxies")) {
        // Get query parameters
        const countryFilter = url.searchParams.get("cc")?.split(",");
        ...
        // Get proxy data with configurations
        const proxyData = await getProxyData(prxList, limit);
        
        // Return JSON response
        return new Response(JSON.stringify({
          success: true,
          total: prxList.length,
          returned: proxyData.length,
          filters: {
      ...
      return new Response(JSON.stringify({
        ...
        endpoints: {
          "/api/proxies": "Get proxy list with filters",
          "/check": "Check proxy health",
          "/api/v1/domains/get": "Get domain list",
...
class CloudflareApi {
  ...
  async registerDomain(domain) {
    ...
    try {
      ...
      // Removed bad words check to simplify
    } catch (e) {
      return 400;
    }
